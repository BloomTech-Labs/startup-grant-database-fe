...
export const useSuggestionActions = () => {
    ...
    const fetchSuggestions = useCallback((token: string, grant_id:number) => {
        dispatch({type: SuggestionTypes.FETCH_SUGGESTION_START});
        axiosWithAuth(token)
            .get(`/grants/${grant_id}`)
            .then((res: AxiosResponse) => {
                dispatch({type: SuggestionTypes.FETCH_SUGGESTION_SUCCESS, payload: res.data});
            })
            .catch((err: AxiosError) => {
                const data =
                    err && err.response && err.response.data ? err.response.data : err;
                dispatch({type: SuggestionTypes.FETCH_SUGGESTION_FAILURE, payload: data});
                    })
            }, [dispatch])

    const selectSuggestion = useCallback((token: string, grant_id: number, id:number) => {
        dispatch({type: SuggestionTypes.SELECT_SUGGESTION_START});
        axiosWithAuth(token)
            .get(`/grants/${grant_id}/suggestion/${id}`)
            .then((res: AxiosResponse) => {
            dispatch({type: SuggestionTypes.SELECT_SUGGESTION_SUCCESS, payload: res.data});
            })
        .catch((err: AxiosError) => {
            const data =
                err && err.response && err.response.data ? err.response.data : err;
            dispatch({type: SuggestionTypes.SELECT_SUGGESTION_FAILURE, payload: data});
            })
    }, [dispatch])

    const deleteSuggestion = useCallback((token: string, grant_id: number, id:number) => {
        ...
        .catch((err: AxiosError) => {
            const data =
                err && err.response && err.response.data ? err.response.data : err;
            dispatch({type: SuggestionTypes.DELETE_SUGGESTION_FAILURE, payload: data});
            })
    }, [dispatch])
};
