...
function NewGrantTable() {
    ...
    function format(grant, column) {
        switch (column.id) {
            ...
            default:
                return grant[column.id] ? moment(grant[column.id]).format('MMM Do YYYY') : null;
        }
        ...
        <Paper className={classes.root}>
            <TableContainer className={classes.container}>
                <Table stickyHeader aria-label="Suggestions Table">
                    ...
                    <TableBody>
                        {grants.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(grant => <GrantTableRow
                            key={grant.id} grant={grant} format={format} columns={columns}/>)}
                    </TableBody>
                </Table>
            </TableContainer>
            <TablePagination
