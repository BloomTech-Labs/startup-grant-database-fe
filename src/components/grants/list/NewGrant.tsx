...
function NewGrant({grant, showcase}: IProps) {
    ...
    const existingFavorite = favoriteGrants.filter((fav: Grant) => fav.id === showcase.id);
        ...
        <Card className={classes.root} raised>
            ...
            <CardActions disableSpacing className={classes.firstActions}>
                <IconDisplay
                    ...
                    existingFavorite={favoriteGrants.filter((favGrant: Grant) => favGrant.id === grant.id)}
                    id={grant.id}
                />
                <IconButton
            ...
            <Collapse in={expanded} timeout="auto" unmountOnExit>
                <CardContent>
                    <Typography paragraph>{`Region: ${grant.geographic_region}`}</Typography>
                    <Typography paragraph>{`Early Stage Funding: ${grant.early_stage_funding}`}</Typography>
                    <Typography paragraph>{`Target Demography: ${grant.target_entrepreneur_demographic}`}</Typography>
                    <Typography paragraph>{`Focus Areas: ${grant.area_focus}`}</Typography>
                    <Typography paragraph>{`Sponsor: ${grant.sponsoring_entity}`}</Typography>
